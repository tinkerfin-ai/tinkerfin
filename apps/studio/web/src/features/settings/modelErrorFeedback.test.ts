...
describe('模型异常全局通知', () => {
  ...
  it('加载失败由统一请求层通知一次，重试成功恢复列表状态', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValueOnce(jsonResponse(null, 503)).mockResolvedValueOnce(jsonResponse([])))
    ...
    hook.rerender()
    expect(apiError).toHaveBeenCalledOnce()
    act(() => hook.result.current.setRevision(value => value + 1))
  ...
  it('测试请求失败由统一请求层通知，业务回调不重复通知', async () => {
    ...
    await act(() => hook.result.current.run('text', {...newModel(), api_key: 'test'}))
    expect(apiError).toHaveBeenCalledOnce()
