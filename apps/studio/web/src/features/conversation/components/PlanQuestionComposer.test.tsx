...
describe('PlanQuestionComposer', () => {
  ...
  it('keeps every option row borderless and on one shared grid', () => {
    ...
    expect(conversationStyles).toMatch(/\.plan-question-composer\s*{[^}]*border:\s*1px solid var\(--color-plan-panel-border\);[^}]*background:\s*var\(--color-layer-1\);/s)
    expect(conversationStyles).toMatch(/\.plan-question-composer-head\s*{[^}]*background:\s*var\(--color-plan-panel-background\);/s)
    expect(conversationStyles).toMatch(/\.plan-question-composer\.is-minimized \.plan-question-composer-head\s*{[^}]*min-height:\s*calc\(var\(--layout-composer-surface-height\) - var\(--control-plan-chip\) - var\(--space-0-5\)\);[^}]*align-items:\s*center;[^}]*padding-top:\s*0;[^}]*padding-bottom:\s*0;/s)
    ...
    expect(conversationStyles).toMatch(/\.plan-question-composer-heading h2,\s*\.plan-review-composer-heading h2\s*{[^}]*font-size:\s*var\(--type-title-size\);[^}]*line-height:\s*var\(--type-title-line\);/s)
