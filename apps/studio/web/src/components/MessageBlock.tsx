...
function RichField({ value, status, className = 'tool-rich-field' }: { value?: string; status?: MessageStatus; className?: string }) {
  return value
    ? <div className={className}><MarkdownContent content={normalizeEscapedText(value)} /></div>
    : <PlaceholderField status={status} />
}
...
function MessageBlockView({
  ...
  if (message.role === 'error') {
    return <article id={message.id} className="error-message"><CircleAlert size={17} /><div><strong>任务遇到问题</strong><MarkdownContent content={message.content} className="error-markdown" /></div></article>
  }
  return (
