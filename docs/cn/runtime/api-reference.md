...
| API | 用途 |
| --- | --- |
| `TinkerFin(checkpointer=..., run_coordinator=..., state_schema=..., runtime_profile=...)` | 配置共享的借用资源 |
...
模型、checkpointer、Store、cache、数据库连接池和服务 client 均由应用打开和关闭。受管运行负责关闭自己创建的 Graph、workspace、流和每次运行资源。
