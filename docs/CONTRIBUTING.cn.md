...
可复现的问题和功能建议请提交到 [Issues](https://github.com/tinkerfin-ai/tinkerfin/issues)。安全漏洞请私下发送至 **1090116461@qq.com**，参见 [SECURITY.md](../SECURITY.md)。

## 提交修改

1. Fork 仓库，从 `main` 创建自己的分支。
2. 每次修改围绕一个明确目标。修复缺陷时增加回归验证，并更新受影响的文档。
3. 向 `tinkerfin-ai/tinkerfin:main` 提交 PR，说明行为变化及执行过的检查。
...
浏览器测试会先构建应用。Docker 集成与独立 wheel 检查见[仓库开发说明](cn/development.md)。未执行的检查请说明原因。
