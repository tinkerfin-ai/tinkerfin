"""Own Messaging work and deliver concurrent failures to its callers."""
...
def retain_failure(primary: BaseException, secondary: BaseException) -> None:
    """Preserve both original error graphs without changing selected precedence."""
    ...
    if additional is not None:
        primary.__cause__ = (
            ...
            if _contains(original, additional)
                ...
                "Messaging operation failures", [original, additional]
            )
        )
...
async def join_owned_task(
    task: asyncio.Future[TaskOutcome[_T]],
    *,
    cancellation: Cancellation | None = None,
) -> _T:
    """Wait for owned work and preserve its complete failure outcome.

    A cancelled waiter cannot abandon an accepted operation or its cleanup.
    Database calls record cancellation before deciding to commit and finish
    consuming driver results. Source cleanup also stays owned until it settles;
    both paths retain concurrent failures until their owner can raise them.
