"""Own SQL State work and deliver concurrent failures to its callers."""
    ...
    if additional is not None:
        primary.__cause__ = (
            ...
            if _contains(original, additional)
                ...
                "Sandbox State operation failures", [original, additional]
            )
        )
...
async def join_owned_task(
    ...
    """Wait for accepted database work and preserve its complete failure outcome.

    State owns startup, worker registration, and close. A cancelled waiter cannot
    cancel this shared work. Writes record cancellation before deciding to commit;
    statement consumption remains owned to avoid abandoning an async driver cursor.
    Engine connection and statement timeouts bound the underlying database I/O.
...
async def run_owned_operation(
    ...
    *,
    ...
    """Own statement consumption and connection return through repeated cancellation."""
