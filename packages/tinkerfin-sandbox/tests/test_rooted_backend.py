...
@pytest.mark.asyncio
async def test_async_cancellation_keeps_worker_handle_lease() -> None:
    old_backend = _BlockingGuardBackend()
    new_backend = _RecordingBackend()
    handle = OpenSandboxHandle(cast(OpenSandboxBackend, old_backend))
    rooted = RootedOpenSandboxBackend(handle)

    operation = asyncio.create_task(rooted.aread("/leased.txt"))
    assert await asyncio.to_thread(old_backend.guard_started.wait, 1)
    operation.cancel()
    with pytest.raises(asyncio.CancelledError):
        await operation

    replaced = handle._replace_backend(cast(OpenSandboxBackend, new_backend))
    wait_until_idle = asyncio.create_task(
        asyncio.to_thread(handle._wait_until_idle, replaced)
    )
    try:
        await asyncio.sleep(0.05)
        assert not wait_until_idle.done()
    finally:
        old_backend.guard_release.set()
...
@pytest.mark.asyncio
async def test_transfer_cancellation_keeps_old_handle_lease_until_settled() -> None:
    class BlockingDescriptorBackend(_RecordingBackend):
        def __init__(self) -> None:
            super().__init__()
    ...
    old_backend = BlockingDescriptorBackend()
    new_backend = _RecordingBackend()
    handle = OpenSandboxHandle(cast(OpenSandboxBackend, old_backend))
    rooted = RootedOpenSandboxBackend(handle)
    operation = asyncio.create_task(rooted.aupload_files([("/leased.bin", b"content")]))
    await old_backend.transfer_started.wait()
    ...
    old_backend.transfer_release.set()
    await wait_until_idle
    for _ in range(100):
    ...
    assert old_backend.rooted_upload_calls == [
        ("/workspace", "/leased.bin", b"content")
    ]
...
@pytest.mark.asyncio
async def test_async_grep_timeout_keeps_worker_handle_lease(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        "tinkerfin_sandbox.backends.rooted.ASYNC_GREP_TIMEOUT",
        0.05,
    )
    old_backend = _BlockingGuardBackend()
    new_backend = _RecordingBackend()
    ...
    operation = asyncio.create_task(rooted.agrep("needle", "/"))
    assert await asyncio.to_thread(old_backend.guard_started.wait, 1)
