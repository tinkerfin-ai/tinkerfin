...
async def test_real_stopped_container_preserves_binding_and_recovers_same_files(
    ...
    async with OpenSandboxManager[str](
        ...
        content = b"isolated-stop-sentinel\x00\xff\n"
        ...
        fault_evidence.record(
            ...
            **{
                f"container_{key}": value
                for key, value in (await fault_docker.snapshot(original_id)).items()
            },
        )
        assert (await fault_docker.change(original_id, "stop"))["status"] == "exited"
        await asyncio.sleep(0.25)
        assert observer.events == []
        fault_evidence.record("no_access", notification_count=0)

        with pytest.raises(OpenSandboxBackendUnavailableError) as captured:
            await manager.get(owner)
        attempts = captured.value.context.get("attempts")
        failure_reason = captured.value.context.get("reason")
        assert isinstance(attempts, int) and not isinstance(attempts, bool)
        assert 1 <= attempts <= OpenSandboxRecoveryPolicy().max_attempts
        assert isinstance(failure_reason, str)
        assert failure_reason in {"unreachable", "timeout"}
        fault_evidence.record(
            "get_failed",
        ...
        assert observer.kinds[0] is Kind.UNAVAILABLE
        assert observer.kinds[-1] is Kind.RECOVERY_FAILED
        assert observer.kinds.count(Kind.RECOVERING) == 1
        assert observer.kinds.count(Kind.RECOVERY_FAILED) == 1
        assert 0 < observer.kinds.index(Kind.RECOVERING) < len(observer.events) - 1
        assert set(observer.kinds) == {
            Kind.UNAVAILABLE,
            Kind.RECOVERING,
            Kind.RECOVERY_FAILED,
        }
        assert all(
            event.reason in {Reason.UNREACHABLE, Reason.TIMEOUT}
            for event in observer.events
        )
        assert observer.events[-1].reason.value == failure_reason
        unavailable_reasons = [
            event.reason for event in observer.events if event.type is Kind.UNAVAILABLE
        ]
        assert unavailable_reasons[-1].value == failure_reason
        assert all(
            previous is not current
            for previous, current in zip(unavailable_reasons, unavailable_reasons[1:])
        )
        fault_evidence.record(
            "outage_sequence", kinds=[kind.value for kind in observer.kinds]
        )

        assert (await fault_docker.change(original_id, "start"))["status"] == "running"
        restored = await manager.get(owner)
        assert restored.id == original_id
        downloaded = (await restored.adownload_files(["/sentinel.bin"]))[0]
        assert downloaded.error is None and downloaded.content == content
        fault_evidence.record(
            "restored",
            sandbox_id=restored.id,
            file_sha256=hashlib.sha256(downloaded.content).hexdigest(),
        )
        await observer.wait_for(Kind.RECOVERED)
        count_after_recovery = len(observer.events)
        for _ in range(2):
            assert (await manager.get(owner)).id == original_id
            assert (await manager.reconnect(owner)).id == original_id
    ...
    assert observer.kinds.count(Kind.RECOVERING) == 1
    assert observer.kinds.count(Kind.RECOVERED) == 1
    assert observer.kinds.count(Kind.DESTROYED) == 1
    _assert_event_identity(observer, owner, original_id)
    _assert_no_named_tasks(fault_evidence)
    fault_evidence.record(
        "final_sequence", kinds=[kind.value for kind in observer.kinds]
    )
...
async def test_real_manager_isolates_observer_failure_and_drains_on_close(
    ...
    """Faulty callbacks cannot change real reset/destroy results or survive close."""

    class FailingObserver:
        """Expose cooperative completion independently of notification diagnostics."""
        ...
        async def on_sandbox_event(self, event: OpenSandboxLifecycleEvent) -> None:
            ...
            try:
                if mode == "raise":
                    raise RuntimeError("synthetic-private-observer-failure")
                if mode == "cancel":
                    raise asyncio.CancelledError("synthetic-private-observer-failure")
    ...
    with caplog.at_level(logging.WARNING, logger="tinkerfin.sandbox.notifications"):
        async with OpenSandboxManager[str](
            ...
            key_resolver=lambda key: key,
            observers=(faulty, observer, *additional_lifecycle_observers),
            notification_options=OpenSandboxNotificationOptions(timeout=0.05),
        ) as manager:
            owner = "observer-owner"
            backend = await manager.get(owner)
            original_id = backend.id
            content = b"callback-failure-sentinel"
            assert (await backend.aupload_files([("/sentinel.bin", content)]))[
                0
            ].error is None
            await manager.reset(owner)
            ...
            missing = (await backend.adownload_files(["/sentinel.bin"]))[0]
            fault_evidence.record(
                "download_after_observer_failure_reset",
                mode=mode,
                sandbox_id=original_id,
                error=missing.error,
                content_is_none=missing.content is None,
    ...
    assert observer.kinds == [Kind.WORKSPACE_RESET, Kind.DESTROYED]
    assert faulty.calls == 2 and faulty.active == 0 and faulty.close_calls == 0
    diagnostic_field = {
        "raise": "tinkerfin_callback_failed",
        "timeout": "tinkerfin_callback_timeout",
        "cancel": "tinkerfin_callback_cancelled",
    }[mode]
    notices = [
        record
    ...
    assert notices
    assert sum(record.__dict__[diagnostic_field] for record in notices) >= 1
    assert "synthetic-private-observer-failure" not in caplog.text
    assert all(record.exc_info is None for record in notices)
    assert (await fault_docker.snapshot(original_id))["status"] == "absent"
    fault_evidence.record(
        "observer_failure_verified",
        mode=mode,
        calls=faulty.calls,
        active=faulty.active,
        observer_close_calls=faulty.close_calls,
        diagnostic_field=diagnostic_field,
        diagnostic_count=sum(record.__dict__[diagnostic_field] for record in notices),
    )
    _assert_event_identity(observer, owner, original_id)
    _assert_no_named_tasks(fault_evidence)
