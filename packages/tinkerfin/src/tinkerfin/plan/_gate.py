...
from ._contracts import GateDecision
...
Never expose private chain-of-thought. Return only the operational goal, route, and up
to three blocking questions when clarification is required. For each question, generate
concise single-select options when they can cover the likely choices, and decide whether
a custom answer is also safe and useful.
"""
    ...
    *,
    ...
    return cast(
        ...
        create_agent(
            ...
            response_format=ToolStrategy(GateDecision.model_json_schema(by_alias=True)),
...
async def invoke_gate(
    ...
    *,
    ...
    result = await agent.ainvoke({"messages": gate_messages})
    response = result.get("structured_response")
    return GateDecision.model_validate(response)
