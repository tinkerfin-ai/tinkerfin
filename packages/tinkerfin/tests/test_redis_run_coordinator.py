...
@pytest.mark.asyncio
async def test_lost_lease_cancellation_survives_a_release_failure() -> None:
    client = _ScriptedRedis(
        [
            1,
            0,
            RedisConnectionError("release unavailable"),
        ]
    )
    coordinator = RedisRunCoordinator.from_client(
        cast(Redis, client),
...
@pytest.mark.asyncio
async def test_unverifiable_renewal_fails_closed_and_cancels_the_owner() -> None:
    client = _ScriptedRedis(
        [
            1,
            RedisConnectionError("renewal unavailable"),
            1,
        ]
    )
    coordinator = RedisRunCoordinator.from_client(
...
@pytest.mark.asyncio
async def test_unexpected_renewal_failure_cancels_the_owner() -> None:
    client = _ScriptedRedis(
        [
            1,
            ValueError("malformed renewal reply"),
            1,
        ]
    )
    coordinator = RedisRunCoordinator.from_client(
