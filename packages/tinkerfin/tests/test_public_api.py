...
def test_generic_object_streams_do_not_claim_a_native_codec_profile() -> None:
    ...
    dataclass_run = TinkerFin[str]().run(dataclass_parts)
    model_run = TinkerFin[str]().run(model_parts)
    assert_type(dataclass_run, TinkerFinRun[_DataclassPart, str])
    assert_type(model_run, TinkerFinRun[_ModelPart, str])
...
def test_strict_native_stream_exposes_a_complete_public_profile() -> None:
    ...
    run = TinkerFin[str]().run(invocation)
    stream = run.astream()

    assert_type(run, NativeTinkerFinRun[str])
